...
1. **Parallelismo per ruolo e per costo** — il vincolo nomina ruoli e fasce di costo, mai un modello: un vincolo che nomina un modello muore col modello.
   ...
   - Nessun lavoro ripetuto: lo stesso task a più agenti di punta, o in più giri, costa più di quanto aggiunge. Due eccezioni: la doppia revisione (regola 9) e il guasto a causa ignota su un task importante — due `debugger` isolati sullo stesso sintomo, poi confronta tu le diagnosi. Isolati apposta: il guadagno viene dall'indipendenza, non dal dibattito.
