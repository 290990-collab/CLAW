...
- **Nessuna scorciatoia sul verde:** mai far passare un controllo indebolendolo — test cancellati o saltati, valore atteso riscritto sull'output, asserzione allargata, stub al posto del codice che deve girare, analisi statica soppressa (`noqa`, `type: ignore`, `any`) o configurazione del linter allentata. Se non passa, si riporta che non passa. Test e specifica in disaccordo: non si allinea uno all'altro in silenzio. Si segnala; senza parola dell'utente vale la specifica. Se a sbagliare è il controllo, lo si corregge in una modifica sua e dichiarata, non lo si aggira nel task che ci è inciampato.
...
- **Livello:** testa dove il difetto può nascere (contratti, confini, invarianti di dominio), preferendo invarianti a esempi. Un test unitario mostra il comportamento di un ramo, non l'assenza del difetto: a provare che il bug è morto è il repro che prima falliva.
