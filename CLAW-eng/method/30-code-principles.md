...
- **No shortcut to green:** never make a check pass by weakening it — deleting or skipping a test, rewriting the expected value to match the output, widening an assertion, stubbing the code that has to run, suppressing static analysis (`noqa`, `type: ignore`, `any`) or loosening the linter configuration. If it does not pass, report that it does not pass. Test and specification in disagreement: one is not silently aligned to the other. Flag it; without the user's word, the specification wins.
